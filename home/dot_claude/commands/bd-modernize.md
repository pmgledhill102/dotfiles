...
### Pre-commit framework interaction (surface as a post-modernise checklist)

If the repo has a `.pre-commit-config.yaml`, the bd-managed files under `.beads/` will trip most lint hooks — `.beads/README.md` is over most line-length limits and has bd-specific vocabulary, `.beads/config.yaml` has no `---` document start, `.beads/hooks/*` source via a `$_lib` variable (shellcheck SC1091).

These files are regenerated by `bd init` every run; fixing them in place is pointless. The right fix is `exclude: ^\.beads/` on the relevant hooks.

After the modernise commit, run:
...
For any hook listed, tell the user: "this repo's pre-commit runs `<hook>` which will fail against the bd-managed `.beads/*` files — you likely want to add `exclude: ^\.beads/` (or extend an existing exclude regex). See `docs/user-manual/beads.md` § Pre-commit framework and beads in pmgledhill102/dotfiles for details." Do not auto-patch the config — every project's hook layout is different.
