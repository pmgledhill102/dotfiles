...
### 5i. Tighten `.beads/` permissions

bd's default `0755` permissions on `.beads/` cause every subsequent bd invocation to print `Warning: .beads has permissions 0755 (recommended: 0700)`. Pure noise — fix it once:
...
echo "===bd-prime hooks (5a verifier)==="; grep -q '"bd prime"' .claude/settings.json 2>/dev/null && echo "OK" || echo "MISSING — Step 5a didn't run; .claude/settings.json should contain SessionStart + PreCompact bd prime entries"
echo "===log==="; git log --oneline -3
```

Expected:

- `bd stats` total matches `$ORIG_LINES` from Step 1 — **minus any `status=tombstone` rows**, which `bd init --from-jsonl` intentionally skips (soft-deletion markers, not live data). Check via `jq -r .status /tmp/beads-${PROJECT}-${TS}.jsonl | sort | uniq -c`; subtract the tombstone count from $ORIG_LINES before comparing.
...
- `.claude/settings.json` contains `bd prime` (Step 5a verifier — proves the post-init hook install actually ran; cloud-coop PR #122 shipped without these because a one-shot pipeline skipped 5a)

### Lint-tool interaction (surface as a post-modernise checklist)

The bd-managed files under `.beads/` will trip most lint hooks — `.beads/README.md` is over most line-length limits and has bd-specific vocabulary, `.beads/config.yaml` has no `---` document start, `.beads/hooks/*` source via a `$_lib` variable (shellcheck SC1091). These files are regenerated by `bd init` every run; fixing them in place is pointless. The right fix is `exclude: ^\.beads/` (or equivalent) on whatever runs the linter.

Two surfaces to check — pre-commit framework AND any GHA workflow that invokes a linter directly. Both need the exclusion; missing either has shipped broken before (cloud-coop PR #122).

```sh
echo "=== pre-commit framework hooks ==="
[ -f .pre-commit-config.yaml ] && grep -E '^\s*- id:' .pre-commit-config.yaml | grep -Ei 'markdownlint|cspell|yamllint|shellcheck' || echo "(no pre-commit lint hooks to worry about)"

echo "=== GHA lint workflows ==="
[ -d .github/workflows ] && grep -lEi 'markdownlint|cspell|yamllint|shellcheck' .github/workflows/*.yml 2>/dev/null || echo "(no GHA lint workflows to worry about)"
```

For each surface that fires:

- **pre-commit hook listed**: tell the user "`<hook>` in `.pre-commit-config.yaml` will fail against `.beads/*` — add `exclude: ^\.beads/` (or extend an existing exclude regex)."
- **GHA workflow listed**: tell the user "`<workflow.yml>` invokes a linter directly — exclude `.beads/` from the lint command's path arguments or add a step-level `paths-ignore` / file-list filter. The pre-commit exclusion does not flow through to GHA-direct invocations."

Do not auto-patch either — every project's hook and workflow layout is different. See `docs/user-manual/beads.md` § Pre-commit framework and beads in pmgledhill102/dotfiles for the pre-commit pattern.

Report a brief summary to the user: which path ran (fast-path vs nuke-and-pave), issue count preserved (and tombstone count elided), any decisions made, and any pre-commit hooks flagged for exclusion.
