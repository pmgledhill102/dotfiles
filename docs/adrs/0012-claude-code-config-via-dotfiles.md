...
- **Status**: Superseded
- **Date**: 2026-04-26 (superseded 2026-05-03)
- **Superseded by**: [ADR-0016](0016-agent-tooling-is-personal-tier.md) —
  agent tooling and its configuration are personal-tier. The agentic config
  moved to its own repo, `agentic-coding-config`, in
  [#203](https://github.com/pmgledhill102/dotfiles/pull/203), which also
  deleted this file; it was restored in
  [#401](https://github.com/pmgledhill102/dotfiles/issues/401) so the number
  stays occupied rather than becoming a silent gap.
- **Tags**: claude-code, tooling

> **Superseded 2026-05-03.** This ADR records why `~/.claude/` was shipped
> from `home/dot_claude/` in this repo. It no longer describes where that
> config lives — `agentic-coding-config` owns it, mounted at `~/.claude/` via
> the archive external in `home/.chezmoiexternal.toml.tmpl`, on `personal`
> machines only. Every `home/dot_claude/` path named below is gone from this
> repo. The one part that stayed is `home/run_onchange_setup-claude.sh`,
> because it is machine bootstrap rather than content.
